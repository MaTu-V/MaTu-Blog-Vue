  ...
  <div class="forget">
    <Form ref="phoneForm" v-if="switchFormStatus" :model="phoneForm" :rules="phoneRuleValidate" :label-width="80">
      ...
      <FormItem class="code-input" label="验 证 码" prop="code">
        ...
        <span v-if="codeStatus" id="code-btn" class="code-btn ivu-tag ivu-tag-success ivu-tag-checked"
      ...
      <FormItem class="bl-button">
        <Button type="success" @click="phoneSubmit(`phoneForm`)"> 验 证</Button>
      </FormItem>
    </Form>
    <Form ref="passwordForm" v-else :model="passwordForm" :rules="passwordRuleValidate" :label-width="80">
      ...
      <FormItem class="bl-button">
        <Button type="success" @click="pwdSubmit(`passwordForm`)"> 修 改</Button>
      </FormItem>
    </Form>
  </div>
</template>
      ...
      getCode(name) {
        ...
        that.$refs[name].validateField('phone', (valid) => {
          if (!valid) {
            let phone = that.encrypt(that.$refs[name].model.phone)
            // 获取验证码
            that.$axios.post('/sms/sendReviseSms', {phone: phone})
              .then(({data: {code}}) => {
                if (code === 1) {
      ...
      pwdSubmit(name) {
        ...
        that.$refs[name].validate((valid) => {
          if (valid) {
            ...
            that.$axios.post('/loginRegister/forgetPwd', {phone: that.phone, password: password})
              .then(({data: {code, msg}}) => {
                if (code === 1) {
                  that.phone = ''
                  that.switchFormStatus = !that.switchFormStatus
